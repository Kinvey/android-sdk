...
/**
 * <p>OfflineStorage class.</p>
 * <p>
 * The instance can be accessed through OfflineStorage.getInstance().
 ...
 * @version $Id: $
 */
public class OfflineStorage<T> extends Observable {
    ...
    private static final long syncRate = 60000;
    ...
    private static HashMap<String, OfflineStorage> storeMap;

    public static OfflineStorage getStore(Context context, String collectionName, Class myClass){
        if (storeMap == null){
            storeMap = new HashMap<String, OfflineStorage>();
        }

        if (!storeMap.containsKey(collectionName)){
            storeMap.put(collectionName, new OfflineStorage(context, collectionName, myClass));
        }
    ...
    public static OfflineStorage getStore(Context context, String collectionName){
        if (storeMap == null){
            storeMap = new HashMap<String, OfflineStorage>();
        }

        if (!storeMap.containsKey(collectionName)){
            storeMap.put(collectionName, new OfflineStorage(context, collectionName));
        }
    ...
    /**
     * <p>Constructor for OfflineStorage.</p>
     *
     * @param context a {@link android.content.Context} object.
     * @param collectionName a {@link java.lang.String} object.
     */
    private OfflineStorage(Context context, String collectionName, Class myClass) {
...
//        this.requestStore = new LinkedList<RequestInfo>();
    ...
    private OfflineStorage(Context context, String collectionName) {
//        this.dataStore = new HashMap<String, T>();
//        this.requestStore = new LinkedList<RequestInfo>();
        this.collectionName = collectionName;
        this.context = context.getApplicationContext();
...
//            }
    ...
    private static void fsyncIfNeeded(ObjectOutputStream os, FileOutputStream out) {
        return;
...
//            if (currentTime - lastSyncAt > syncRate) {
//                try {
//                    os.flush();
//                    out.getFD().sync();
//                } catch (IOException ioe) {
//                    Log.e(Client.TAG, "Error flushing output stream!");
//                }
