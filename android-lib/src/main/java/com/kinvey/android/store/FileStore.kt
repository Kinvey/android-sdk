...
class FileStore(networkFileManager: NetworkFileManager,
    ...
    private enum class FileMethods {
        UPLOAD_FILE,
    ...
    fun upload(file: File, listener: AsyncUploaderProgressListener<FileMetaData>) {
        AsyncUploadRequest(this, asyncMethods[FileMethods.UPLOAD_FILE], listener, file).execute()
    }
    ...
    fun upload(file: File, metadata: FileMetaData,
        ...
        AsyncUploadRequest(this, asyncMethods[FileMethods.UPLOAD_FILE_METADATA], listener,
                file, metadata).execute()
    }

    @Throws(IOException::class)
    fun upload(`is`: InputStream, metadata: FileMetaData, listener: AsyncUploaderProgressListener<FileMetaData>) {
        AsyncUploadRequest(this, asyncMethods[FileMethods.UPLOAD_STREAM_METADATA], listener,
                `is`, metadata).execute()
    }
    ...
    fun upload(filename: String, `is`: InputStream,
               listener: AsyncUploaderProgressListener<FileMetaData>) {
        AsyncUploadRequest(this, asyncMethods[FileMethods.UPLOAD_STREAM_FILENAME], listener,
                filename, `is`).execute()
    }

    @Throws(IOException::class)
    fun remove(metadata: FileMetaData, callback: KinveyDeleteCallback) {
        AsyncRequest(this, asyncMethods[FileMethods.REMOVE_ID], callback,
                metadata).execute()
    }
    ...
    fun download(metadata: FileMetaData, os: OutputStream,
        ...
        AsyncDownloadRequest(this, asyncMethods[FileMethods.CACHED_DOWNLOAD_METADATA], progressListener,
    ...
    fun refresh(metadata: FileMetaData, metaCallback: KinveyClientCallback<FileMetaData>, cachedClientCallback: KinveyCachedClientCallback<FileMetaData>? = null) {
        AsyncRequest(this, asyncMethods[FileMethods.REFRESH_FILE], metaCallback, metadata, cachedClientCallback).execute()
    }
    ...
    fun find(q: Query, metaCallback: KinveyClientCallback<Array<FileMetaData>>, cachedClientCallback: KinveyCachedClientCallback<Array<FileMetaData>>? = null) {
        AsyncRequest(this, asyncMethods[FileMethods.FIND_QUERY], metaCallback, q, cachedClientCallback).execute()
    }
    ...
    companion object {
        ...
        private val asyncMethods = HashMap<FileMethods, Method>()

        init {
            try {
                //UPLOAD METHODS
                asyncMethods[FileMethods.UPLOAD_FILE] = BaseFileStore::class.java.getDeclaredMethod("upload", File::class.java, UploaderProgressListener::class.java)
                asyncMethods[FileMethods.UPLOAD_FILE_METADATA] = BaseFileStore::class.java.getDeclaredMethod("upload", File::class.java,
                        FileMetaData::class.java,
                        UploaderProgressListener::class.java)
                asyncMethods[FileMethods.UPLOAD_STREAM_METADATA] = BaseFileStore::class.java.getDeclaredMethod("upload", InputStream::class.java,
                        FileMetaData::class.java,
                        UploaderProgressListener::class.java)
                asyncMethods[FileMethods.UPLOAD_STREAM_FILENAME] = BaseFileStore::class.java.getDeclaredMethod("upload", String::class.java,
                        InputStream::class.java,
                        UploaderProgressListener::class.java)

                //DELETE

                asyncMethods[FileMethods.REMOVE_ID] = BaseFileStore::class.java.getDeclaredMethod("remove", FileMetaData::class.java)

                //DOWNLOAD

                asyncMethods[FileMethods.DOWNLOAD_METADATA] = BaseFileStore::class.java.getDeclaredMethod("download", FileMetaData::class.java, OutputStream::class.java, KinveyCachedClientCallback::class.java, DownloaderProgressListener::class.java)

                asyncMethods[FileMethods.CACHED_DOWNLOAD_METADATA] = BaseFileStore::class.java.getDeclaredMethod("download", FileMetaData::class.java, OutputStream::class.java, OutputStream::class.java, KinveyCachedClientCallback::class.java, DownloaderProgressListener::class.java)

                //REFRESH
                asyncMethods[FileMethods.REFRESH_FILE] = BaseFileStore::class.java.getDeclaredMethod("refresh", FileMetaData::class.java, KinveyCachedClientCallback::class.java)

                //FIND
                asyncMethods[FileMethods.FIND_QUERY] = BaseFileStore::class.java.getDeclaredMethod("find", Query::class.java, KinveyCachedClientCallback::class.java)
