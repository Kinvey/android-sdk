...
public abstract class AbstractSyncService extends IntentService{
    ...
    public void getFromStoreAndExecute() {
        new AsyncTask<Void, Void, Void>(){
            ...
            protected Void doInBackground(Void... voids) {
                SyncManager syncManager = getSyncManager(client);
                List<String> collectionNames = syncManager.getCollectionTables();
                ...
                for (int i = 0; i < client.getBatchSize(); i++){
                    for (String s : collectionNames) {
                        ...
                        if (safeToAttempExecution()){
                            SyncRequest req = syncManager.popSingleQueue(s, syncManager);
                            if (req == null){
                                done = true;
                            }else{
                                executeRequest(syncManager, req, s);
                            }
                        }else{
     ...
     *
     * @param syncManager wrapper of database implementation
     * @param cur the current offline request to be executed
     * @param collectionName the name of the collection for this request
     */
    private void executeRequest(final SyncManager syncManager, final SyncRequest cur, final String collectionName) {
        if (cur.getHttpVerb().equals(SyncRequest.HttpVerb.PUT) || cur.getHttpVerb().equals((SyncRequest.HttpVerb.POST))) {
        	...
        	final GenericJson entity = syncManager.getEntity(client, cur.getEntityID());
        	//grab entity's id
            ...
            if (entity != null){
                ...
                client.dataStore(collectionName, GenericJson.class).save(entity, new KinveyClientCallback<GenericJson>() {
                    ...
                    public void onSuccess(GenericJson result) {
                    	...
                    	if (curEntityID.startsWith(AbstractKinveyJsonClientRequest.TEMPID)){
                    		syncManager.removeEntity(collectionName, curEntityID);
                    	}
                    	//save the updated entity
                        syncManager.insertEntity(collectionName, client, result);
                    }
                    ...
                    public void onFailure(Throwable error) {
                        ...
                        syncManager.storeCompletedRequestInfo(collectionName, false, cur, error);
                    }
                });
            }else{
            	//couldn't load entity needed to perform save request: there is no local copy of the entity to save remotely.   
                syncManager.storeCompletedRequestInfo(collectionName, false, cur, new NullPointerException());
            }
        } else if (cur.getHttpVerb().equals(SyncRequest.HttpVerb.GET)){
        	client.dataStore(collectionName, GenericJson.class).setClientAppVersion(cur.getEntityID().customerVersion);
        	client.dataStore(collectionName, GenericJson.class).setCustomRequestProperties(new Gson().fromJson(cur.getEntityID().customheader, GenericJson.class));
...
//                    KinveySyncService.this.storeCompletedRequestInfo(collectionName, true, cur, result);
        ...
        } else if (cur.getHttpVerb().equals(SyncRequest.HttpVerb.DELETE)){
        	client.dataStore(collectionName, GenericJson.class).setClientAppVersion(cur.getEntityID().customerVersion);
        	client.dataStore(collectionName, GenericJson.class).setCustomRequestProperties(new Gson().fromJson(cur.getEntityID().customheader, GenericJson.class));
...
//                        KinveySyncService.this.storeCompletedRequestInfo(collectionName, true, cur, result);
        ...
        }else if (cur.getHttpVerb().equals(SyncRequest.HttpVerb.QUERY)){
            ...
            try{
                ...
                for (GenericJson res : result){
                    ...
                    //update datastore with response
                    syncManager.insertEntity(collectionName, client, res);
                    resultIds.add(res.get("_id").toString());
                }
                syncManager.storeQueryResults(collectionName, cur.getEntityID().id, resultIds);
            }catch(Exception e){
                syncManager.storeCompletedRequestInfo(collectionName, false, cur, e);
            }
     ...
     */
    private void storeCompletedRequestInfo(final String collectionName,final boolean success, final SyncRequest info,final Throwable error) {
        ...
        new AsyncTask<Void, Void, Void>(){
            ...
            protected Void doInBackground(Void... voids) {
                ...
                if (!success && error != null && !(error instanceof HttpResponseException)){
                    ...
                    SyncManager syncManager = getSyncManager(client);
                    syncManager.enqueueRequest(collectionName, info.getHttpVerb(), info.getEntityID());
                    registerFailure();
                }else{
    ...
    protected abstract SyncManager getSyncManager(Client client);
